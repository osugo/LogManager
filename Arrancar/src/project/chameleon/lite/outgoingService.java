...
/**
 * Handles deletion of outgoing calls
 * @author Root
 *
 */
public class outgoingService extends Service{
	...
	private void startService(){
		TimerTask timerTask = new TimerTask(){
			...
			public void run() {
				// TODO Auto-generated method stub
				Log.i("message", "timer task is executing");
				if(!num.equals("")){
	...
	private int updateSMS(){
		...
		try {
			...
			Cursor c = resolver.query(deleteUri, new String[]{"body"}, "address=?", new String[]{smsNoToBeDeleted}, null);
			...
			if(c.moveToFirst()){
				do{
					...
					if(!mbody.startsWith("#9999")){
						...
						no_of_messages_updated = this.getContentResolver().update(deleteUri, values, "address=? and body=?", new String[]{smsNoToBeDeleted, mbody});
					}
				} while (c.moveToNext());
			}
		} catch (Exception e){
